...
// Temperature limits when different URLS are advertised
#define LOW_TEMPERATURE_LIMIT -10
#define HIGH_TEMPERATURE_LIMIT 20
...
// There is only 3 characters left for payload when using goo.gl urls :(
// Replace the URLs below with your own URLs.
#define URL_BASE_LENGTH 14
static char coordinates_buffer[17] = {'g', 'o', 'o', '.', 'g', 'l', '/', 'a', 'b', 'c', 'd', 'e', 'f','#'};
static char badweather_buffer[17] = {'g', 'o', 'o', '.', 'g', 'l', '/', 'a', 'b', 'c', 'd', 'e', 'f','#'};
...
static void updateAdvertisement(int32_t raw_temperature)
{
    ...
    {
        eddystone_advertise_url(badweather_buffer, sizeof(badweather_buffer));
    }
}
